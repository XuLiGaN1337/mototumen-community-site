...
const sections: Section[] = [
  {
    ...
    content: (
      <div className="space-y-6">
        <p className="text-zinc-400">Каждый пользователь на платформе имеет основную роль и может дополнительно получить статус организации.</p>
          ...
          {[
            {
              ...
              can: ["Просматривать профили, магазины, мотошколы, сервисы", "Публиковать объявления на мото-авито", "Добавлять технику в гараж", "Добавлять места в избранное", "Добавлять друзей", "Участвовать в жизни сообщества"],
            },
            {
              emoji: "🛡️",
              role: "Модератор",
            ...
            },
            {
              ...
              can: ["Всё, что может модератор", "Управление пользователями и ролями", "Редактирование контента сайта", "Доступ к панели администратора"],
            },
            {
              emoji: "👑",
              role: "CEO",
              ...
              can: ["Полный доступ ко всем функциям", "Одобрение и отклонение заявок на организацию", "Управление ролями всех пользователей", "Удаление любого контента"],
            },
            {
              emoji: "🏢",
              role: "Организация (доп. статус)",
              badge: "bg-blue-900/50 text-blue-400",
              desc: "Дополнительный статус — работает вместе с любой другой ролью. Означает, что у пользователя есть одобренная организация на платформе.",
              can: ["Управление своей организацией (редактирование данных)", "Добавление карточек товаров, услуг или курсов", "Отображение в каталоге магазинов, сервисов или мотошкол", "Бейдж 🏢 и название организации в профиле"],
            },
          ].map((item) => (
        ...
        {[
          {
            ...
            steps: [
              ...
              "Выберите способ: через Telegram, Яндекс или email/пароль",
              "После входа вы получаете роль «Пользователь»",
              "Заполните профиль: фото, позывной, гараж",
            ],
          },
          {
            icon: "Building2",
          ...
          },
          {
            ...
            steps: [
              ...
              "Нажмите кнопку «Подать объявление» внизу экрана",
              "Выберите категорию, заполните данные и укажите контакт (Telegram)",
              "Объявление сразу появляется в разделе «Опубликованные»",
              "Свои объявления можно просматривать и удалять во вкладке «Мои объявления»",
            ],
          },
          {
            icon: "Users",
            title: "Друзья и гараж",
            steps: [
              "Найдите участника в сообществе и откройте его профиль",
              "Нажмите «Добавить в друзья» — он получит уведомление",
              "После принятия заявки вы видите друг друга в списке друзей",
              "В «Гараже» можно добавить свою технику: марку, модель, год выпуска и фото",
            ],
          },
        ].map((block) => (
  ...
  },
  {
    ...
    content: (
      <div className="space-y-4">
        <p className="text-zinc-400">Платформа объединяет три типа организаций мотосообщества:</p>
        {[

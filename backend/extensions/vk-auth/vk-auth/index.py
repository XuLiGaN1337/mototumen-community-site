        ...
        try:
            ...
            if row:
                ...
                if vk_email and row:
                    ...
                    # 3. Create new user
                    safe_email = vk_email or f"vk_{vk_user_id}@vk.user"
                    safe_name = full_name or f"VK User {vk_user_id}"
                    cur.execute(
                        f"""INSERT INTO {S}users
                            (vk_id, email, name, avatar_url, password_hash, role, is_hidden, created_at, updated_at)
                            VALUES (%s, %s, %s, %s, '', 'user', false, NOW(), NOW())
                            RETURNING id""",
                        (str(vk_user_id), safe_email, safe_name, photo_url)
                    )

    ...
    try:
        ...
        if method == 'GET' and action == 'public-products':
            ...
            cur.execute(f"""
                SELECT id, name, description, price, image_url as image,
                       category, in_stock as "inStock", brand, model, discount, stock_quantity as "stockQuantity", created_at
        ...
        if action in ('', 'products') or not action:
            ...
            if method == 'POST':
                ...
                stock_qty = max(0, int(b.get('stockQuantity', 0) or 0))
                cur.execute(f"""
                    INSERT INTO {SCHEMA}.products
                    (name, description, price, image_url, category, in_stock, brand, model, discount, stock_quantity, shop_id)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id
                """, (
                    b.get('name'), b.get('description', ''), b.get('price'),
                    b.get('image', ''), b.get('category', ''), b.get('inStock', True),
                    b.get('brand', ''), b.get('model', ''), discount, stock_qty, shop_id
                ))
            ...
            if method == 'PUT':
                ...
                cur.execute(f"""
                    ...
                    SET name=%s, description=%s, price=%s, image_url=%s,
                        category=%s, in_stock=%s, brand=%s, model=%s, discount=%s, stock_quantity=%s, updated_at=NOW()
                    WHERE id=%s AND shop_id=%s
                """, (
                    b.get('name'), b.get('description', ''), b.get('price'),
                    b.get('image', ''), b.get('category', ''), b.get('inStock', True),
                    b.get('brand', ''), b.get('model', ''), discount, stock_qty, pid, shop_id
                ))
                conn.commit()
